# Day04 整理版｜字典、集合与循环 else

> 这一页是 Day04 的复习主入口。原始页面保留作为学习记录，这里把机翻术语统一为正常中文，并把相关方法放回对应知识点下面。
...
> [!note] 术语修正
> 原笔记中的“词典”“密钥”都是机翻。Python 常用中文术语是 **字典** 和 **键**。
...
- 放入集合的元素必须是可哈希的，例如数字、字符串、元组通常可以；列表、字典不能直接作为集合元素。
...
> [!warning] 原笔记的一处机翻错误
> “集合是一个无序的不可变元素列表”不准确。应该理解为：**集合是可变容器，但集合中的元素必须可哈希；集合不允许重复元素。**
